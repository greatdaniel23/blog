...
/**
 * Preserve /en/* requests for Astro's dedicated English route files and set
 * X-Locale: en so middleware and shared components can select English copy.
 *
 * Architecture (ref src/i18n/utils.ts):
 *   worker.ts preserves /en/* and sets X-Locale: en header
 *   middleware.ts reads the header and sets locals.locale
 *   Pages/components read locals.locale and call t(locale, key)
 */
function markEnglishRequest(request: Request): Request {
	...
	return {
		default: {
			async fetch(request: Request, env: any, context: any): Promise<Response> {
				...
				// Preserve /en/ so Astro matches the dedicated English route files.
				if (incoming.pathname === '/en' || incoming.pathname.startsWith('/en/')) {
					return handle(manifest, app, markEnglishRequest(request), env, context);
				}
